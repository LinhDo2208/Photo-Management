...
Pnode sortingDateCreate(Linkedlist H, int mode)
{
    // mode = 1 sắp xếp tăng dần, mode = 2 sắp xếp giảm dần
    Pnode p, q;
    for (p = H; p != NULL; p = p->next)
    {
        for (q = p->next; q != NULL; q = q->next)
        {
            int cmp = compareDate(p->data.datec, q->data.datec);
...
Pnode sortingDateModify(Linkedlist H, int mode)
{
    // mode = 1 sắp xếp tăng dần, mode = 2 sắp xếp giảm dần
    Pnode p, q;
    for (p = H; p != NULL; p = p->next)
    {
        for (q = p->next; q != NULL; q = q->next)
        {
            int cmp = compareDate(p->data.datem, q->data.datem);
            if (((mode == 1) && (cmp <= 0)) || ((mode == 2) && (cmp >= 0)))
            {
                swap(p, q);
            }
...
Pnode sortingDateAccess(Linkedlist H, int mode)
{
    // mode = 1 sắp xếp tăng dần, mode = 2 sắp xếp giảm dần
    Pnode p, q;
    for (p = H; p != NULL; p = p->next)
    {
        for (q = p->next; q != NULL; q = q->next)
        {
            int cmp = compareDate(p->data.datea, q->data.datea);
            if (((mode == 1) && (cmp <= 0)) || ((mode == 2) && (cmp >= 0)))
            {
                swap(p, q);
            }
...
{
    // mode = 1 sắp xếp từ A-Z, mode = 2 sắp xếp từ Z-A
    Pnode p, q;
    for (p = H; p != NULL; p = p->next)
    {
        for (q = p->next; q != NULL; q = q->next)
...
int main()
{
    ...
    while (choice != 0)
    {
        ...
        case 6:
        {
            ...
            showRubbishlist(R);
            int choice_rub = -1;
            while (choice_rub != 0) // Changed from != 3 to != 0
            {
                printf("\n1. Restore");
                ...
                case 1:
                {
                    ...
                    {
                        printf("This file already exists in your list. Try again!\n");
                    }
                    else
                    {
        ...
        case 7:
        {
            ...
            while (sort != 6)
            {
                ...
                printf("\n3. Sort by date of modification");
                printf("\n4. Sort by date of access");
                printf("\n5. Sort by size");
                printf("\n0. Back");
                printf("\nChoice: ");
                scanf("%d", &sort);
                ...
                case 3:
                {
                    ...
                    {
                        printf("Sort by date of modification in ascending order.\n");
                        H = sortingDateModify(H, modem);
                        showlist(H);
                        break;
                    }
                    else if (modem == 2)
                    {
                        printf("Sort by date of modification in descending order.\n");
                        H = sortingDateModify(H, modem);
                        showlist(H);
                        break;
                    }
                ...
                case 5:
                {
                    ...
                    {
                        printf("Sort by size in ascending order.\n");
                    ...
                    {
                        printf("Sort by size in descending order.\n");
                        H = sortingSize(H, modes);
                        showlist(H);
                        break;
                    }
